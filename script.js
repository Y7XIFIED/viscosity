...
(() => {
  ...
  const THEMES = [
    { bg: [0.0, 0.0, 0.0], fill: [0.8, 1.0, 0.0], css: "#c8ff00" }, // Neon Green on Black
    { bg: [0.0, 0.0, 0.0], fill: [0.765, 0.518, 1.0], css: "#c284ff" }, // Lavender on black
    { bg: [0.0, 0.0, 0.0], fill: [0.0, 0.8, 1.0], css: "#00ccff" }, // Cyan on black
    { bg: [0.0, 0.0, 0.0], fill: [1.0, 0.2, 0.3], css: "#ff334d" }  // Neon red on black
  ];
  ...
  function init() {
    ...
    function buildMask(text) {
      ...
      pc.fillStyle = "#fff";
      pc.font = `900 ${MASK.PROBE_FONT_SIZE}px Nasalization, sans-serif`;
      pc.textAlign = "center";
      ...
      maskCtx.font = `900 ${finalFontSize}px Nasalization, sans-serif`;
    ...
    const uniforms = {
      uTime: { value: 0.0 },
      uColorBg: { value: new Vector3(...THEMES[0].bg) },
      uColorFill: { value: new Vector3(...THEMES[0].fill) },
      uRes: {
        value: new Vector2(
    ...
    const fragmentShader = /* glsl */ `
      ...
      #define COLOR_BG uColorBg
      #define COLOR_FILL uColorFill
...
${TURB_WAVES.map(turbWaveGLSL).join("\n")}
    ...
    document.getElementById("btn-theme").addEventListener("click", function () {
      ...
      uniforms.uColorBg.value.set(...t.bg);
      uniforms.uColorFill.value.set(...t.fill);
      document.documentElement.style.setProperty('--color-accent', t.css);
      document.documentElement.style.setProperty('--color-bg', '#000000');
    });

    // Initialize CSS vars
    document.documentElement.style.setProperty('--color-accent', THEMES[0].css);
    document.documentElement.style.setProperty('--color-bg', '#000000');
